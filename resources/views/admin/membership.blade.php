...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            // 2. TOMBOL SIMPAN (CREATE / UPDATE)
            $('#btnSave').click(function() {
                let id = $('#id').val();

                // PERBAIKAN PENTING DI SINI:
                // Mengganti route 'kasir.' menjadi 'admin.' sesuai layout Anda
                let url = id ?
                    '{{ route('admin.customers.update', ':id') }}'.replace(':id', id) :
                ...
                // Hanya kirim password jika field diisi
                if (!id || $('#password').val()) {
                ...
                $.ajax({
                    url: url,
                    type: 'POST', // Gunakan POST, Laravel akan membaca _method: PUT
                    data: dataKirim,
            ...
            // 3. TOMBOL EDIT (Mengambil Data)
            $(document).on('click', '.btnEdit', function() {
                let id = $(this).data('id');

                // PERBAIKAN: Gunakan route ADMIN
                let url = '{{ route('admin.customers.show', ':id') }}'.replace(':id', id);
                ...
                $.get(url, function(data) {
                    ...
                    // Cek apakah email ada di tabel customers atau di relasi user
                    let email = data.user ? data.user.email : data.email;
                    $('#email').val(email);
                    ...
                    console.log(xhr.responseText); // Lihat error di Console Browser
                    alert('Gagal mengambil data. Pastikan Controller mereturn JSON, bukan View. Error: ' +
                        xhr.status);
                });
            });
            ...
            $(document).on('click', '.btnDelete', function() {
                let id = $(this).data('id');
                // PERBAIKAN: Gunakan route ADMIN
                let url = '{{ route('admin.customers.destroy', ':id') }}'.replace(':id', id);
